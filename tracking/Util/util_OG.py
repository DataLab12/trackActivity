...
def draw_bboxes(img, bbox, dists, identities=None, offset=(0,0)):
    ...
    for i,box in enumerate(bbox):
        ...
        #     gps_x = x1 - 427
        #     gps_y = 480 - y1
        print("id: " + str(identities[i]) + " | " + "dist = " + str(dists[i]) + " | " + "X/Y Coordinates: " + str(gps_x) + " / " + str(gps_y))
        color = COLORS_10[id%len(COLORS_10)]
        label = '{} {}'.format("Boat", id)
        d_label = '{} {}'.format("Distance", dists)
        gps_label = '{} {} {}'.format("Coordinate", gps_x,  gps_y)
        ...
        cv2.putText(img,d_label,(x2,y2-t_size[1]-4 - myoffset), cv2.FONT_HERSHEY_PLAIN, 1, [255,255,255], 2)
        cv2.putText(img, gps_label,(x2,y2-t_size[1]-1 + (-10)), cv2.FONT_HERSHEY_PLAIN, 1, [255,255,255], 2)
    return img


def writecsv():
    zdata = np.array(z_cord)
    xdata = np.array(x_cord)
    id_data = np.array(id_cur)
    output = np.column_stack((xdata,zdata))
    output2 = np.column_stack((output,id_data))
    output_fin = np.column_stack((output2, d_list))
    np.savetxt('s1t1Cam14_AltCord.csv', output_fin, delimiter=',', fmt=['%s', '%s', '%s', '%s'], header='XCord,ZCord,ID,Dist', comments='')
